...
namespace SAM.Geometry.Grasshopper.Solver
{
    ...
    public class GrasshopperSnapSolver
    {
        ...
        public GrasshopperSnapSolver(List<Brep> PanelsBrepInput, List<double> BucketSizesInput, List<double> WeightsInput, List<double> MaxExtensionsInput,
        ...
        public void Execute()
        {
            ...
            List<GrasshopperSnappedWall> walls = RegisterWalls(PanelsBrep, BucketSizes, Weights, MaxExtensions, Levels, LevelSectionOffset, _extensionLimiter);
            List<GrasshopperSnappedWall> snapped = SnapAndAdjustWalls(walls);
            ...
            SortedList<double, List<GrasshopperSnappedWall>> snappedWallsPerFloor = SortWallsByElevation(snapped);
            ...
            foreach (double level in snappedWallsPerFloor.Keys)
            {
                ...
                List<GrasshopperSnappedWall> currentFloor = snappedWallsPerFloor[level];
                int segmentCount = 0;
        ...
        private static SortedList<double, List<GrasshopperSnappedWall>> SortWallsByElevation(List<GrasshopperSnappedWall> walls)
        {
            SortedList<double, List<GrasshopperSnappedWall>> snappedWallsPerFloor = new SortedList<double, List<GrasshopperSnappedWall>>();
            for (int i = 0; i < walls.Count; i++)
            {
                double currentLevel = Math.Round(walls[i].Elevation, ElevationToleranceDigits);
                if (!snappedWallsPerFloor.ContainsKey(currentLevel))
                {
                    snappedWallsPerFloor[currentLevel] = new List<GrasshopperSnappedWall>();
                }
                snappedWallsPerFloor[currentLevel].Add(walls[i]);
            }
            return snappedWallsPerFloor;
        }
        private static void MarkNakedNodes(List<GrasshopperSnappedWall> walls)
        {
            for (int i = 0; i < walls.Count; i++)
            {
        ...
        private static List<GrasshopperSnappedWall> CreateGraph(List<GrasshopperSnappedWall> walls, double snappingDistance)
        {
            GrasshopperGraphSolver solver = new GrasshopperGraphSolver(walls.Select(w => w.ProjectedAxis).ToList(), 
            ...
            List<GrasshopperSnappedWall> graph = new List<GrasshopperSnappedWall>();
            // different levels are taken into consideration
            for (int i = 0; i < newAxes.Count; i++)
            {
                Line currentAxis = newAxes[i];
                List<GrasshopperSnappedWall> representedByThisAxis = new List<GrasshopperSnappedWall>();
                foreach (int j in sourceIndices[i])
                {
                    representedByThisAxis.Add(walls[j]);
                }
                SortedList<double, List<GrasshopperSnappedWall>> thisAxisSortedByLevels = SortWallsByElevation(representedByThisAxis);
                ...
                foreach (var floor in thisAxisSortedByLevels)
                {
                    ...
                    GrasshopperSnappedWall mergedWall = new GrasshopperSnappedWall(sources[0], axis, weight, bucketSize, maxExtension, height);
        ...
        private static List<GrasshopperSnappedWall> MergeColinearWalls(List<GrasshopperSnappedWall> walls)
        {
            double angleRadTol = 0.01;
            double distTol = 0.001;

            SortedList<double, List<GrasshopperSnappedWall>> sortedByElevation = SortWallsByElevation(walls);

            List<GrasshopperSnappedWall> merged = new List<GrasshopperSnappedWall>();
            foreach (var floor in sortedByElevation)
            {
                List<GrasshopperSnappedWall> thisLevelWalls = floor.Value;
                ...
                for (int i = 0; i < thisLevelWalls.Count; i++)
                {
                    ...
                    processed[i] = true;
                    GrasshopperSnappedWall masterWall = thisLevelWalls[i];
                    ...
                    GrasshopperSnappedWall mergedWall = new GrasshopperSnappedWall(masterWall.SourceIndices[0], masterAxis, masterWall.Weight, masterWall.BucketSize, masterWall.MaxExtension, masterWall.OriginalHeight);
        ...
        private static List<GrasshopperSnappedWall> ExplodeWallsAtIntersections(List<GrasshopperSnappedWall> walls)
        {
            List<GrasshopperSnappedWall> split = new List<GrasshopperSnappedWall>();
            ...
            for (int i = 0; i < walls.Count; i++)
            {
                ...
                for (int j = 0; j < walls.Count; j++)
                {
                    ...
                    if (!RhinoMath.EpsilonEquals(walls[i].Elevation, walls[j].Elevation, ModelTolerance))
                    ...
                    currentExtended.Extend(ModelTolerance, ModelTolerance);
                    otherExtended.Extend(ModelTolerance, ModelTolerance);
                    if (global::Rhino.Geometry.Intersect.Intersection.LineLine(currentExtended, otherExtended, out myParam, out theirParam, SAMTolerance, finiteSegments: true))
                    {
        ...
        private static void SnapOpenNodes(List<GrasshopperSnappedWall> walls, double snappingDistance)
        {
            List<GrasshopperSnappedWall> wallsByLength = walls.OrderBy(w => w.Length).ToList(); // start snapping from the shortest
            for (int i = 0; i < wallsByLength.Count; i++)
            {
                GrasshopperSnappedWall currentWall = wallsByLength[i];
                ...
                {
                    if (!RhinoMath.EpsilonEquals(currentWall.Elevation, wallsByLength[j].Elevation, ModelTolerance)) // same level only
                    {
                        continue;
                    }
                    if (i == j)
        ...
        private static List<GrasshopperSnappedWall> SnapAndAdjustWalls(List<GrasshopperSnappedWall> walls)
        {
            ...
            List<GrasshopperSnappedWall> processedWalls = new List<GrasshopperSnappedWall>(walls);
            ...
            do
            {
                ...
                {
                    GrasshopperSnappedWall current = processedWalls[i];
                    for (int j = i + 1; j < processedWalls.Count; j++)
                    {
                        ...
                        GrasshopperSnappedWall candidate = processedWalls[j];
                        bool mergedIn = false;
                ...
                List<GrasshopperSnappedWall> allButMerged = new List<GrasshopperSnappedWall>();
        ...
        private static void TrimAndExtendWalls(List<GrasshopperSnappedWall> walls)
        {
            ...
            foreach (var floor in wallsPerFloor)
            {
                ...
                GrasshopperExtensionSolver solver = new GrasshopperExtensionSolver(axes, extensions, SAMTolerance);
        ...
        private static List<GrasshopperSnappedWall> RegisterWalls(List<Brep> panelsBrep, List<double> bucketSizes, List<double> weights, List<double> maxExtensions, List<Interval> levels, double levelOffset, double extensionLimiter)
        {
            var walls = new List<GrasshopperSnappedWall>();
            ...
            for (int j = 0; j < levels.Count; j++)
            {
                ...
                for (int i = 0; i < panelsBrep.Count; i++)
                {
                    ...
                    if (global::Rhino.Geometry.Intersect.Intersection.BrepPlane(thiswall, currentPlane, ModelTolerance, out intc, out intp))
                    {
                        foreach (Curve curve in intc)
                        {
                            if (curve.IsLinear(ModelTolerance))
                            {
                                Line section = new Line(curve.PointAtStart, curve.PointAtEnd);
                                GrasshopperSnappedWall wall = new GrasshopperSnappedWall(i, section, weights[i], bucketSizes[i], Math.Min(maxExtensions[i], section.Length * extensionLimiter), currentHeight);
                                walls.Add(wall);
                            }
                        }
                    }
