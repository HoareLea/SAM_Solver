    ...
    public static partial class Query
    {
        public static List<Line> FixSlab(List<Line> slabOutline, double tolerance)
        {
            ...
            for (int i = 0; i <= slabOutline.Count - 1; i++)
            {
                ...
                if (global::Rhino.Geometry.Intersect.Intersection.LineLine(thisline, thatline, out thisparam, out thatparam, tolerance, false))
                    thisline.To = thisline.PointAt(thisparam);
                else
                    thisline.To = thatline.From;

                if (global::Rhino.Geometry.Intersect.Intersection.LineLine(thisline, prevline, out thisparam, out thatparam, tolerance, false))
